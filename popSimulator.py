...
tree_sequence = msprime.sim_ancestry(samples=sample_size, ploidy=2, population_size=Ne, sequence_length=loci, random_seed=1234)
...
def format_diploid_haplotypes(haplotypes):
...
diploid_haplotypes = format_diploid_haplotypes(haplotypes)

# Encode haplotypes to
encoding = {'N': '00', 'A': '01', 'G': '02', 'C': '03', 'T': '04'}

def encode_haplotypes(hap):
    encoded_values = [encoding[char] for char in hap]
...
for i, hap in enumerate(diploid_haplotypes):
    encoded_haplotypes = encode_haplotypes(hap)
    content += f"{i+1} , {encoded_haplotypes}\n"
